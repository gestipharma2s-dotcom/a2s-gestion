  ...
  async getCurrentUser() {
    try {
      ...
      try {
        ...
        console.warn('⚠️ Erreur parsing localStorage:', parseError);
        // Essayer de récupérer depuis Supabase en dernier recours
        try {
          const { data: { user: supabaseUser }, error } = await supabase.auth.getUser();
          if (!error && supabaseUser) {
            console.log('✅ Utilisateur récupéré depuis Supabase');
            return {
              user: supabaseUser,
          ...
          console.warn('⚠️ Impossible de récupérer depuis Supabase');
        }
        return null;
      }
    } catch (error) {
      console.error('Erreur critique récupération utilisateur:', error);
      return null;
    }
  },
