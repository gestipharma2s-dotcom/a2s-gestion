...
const InterventionJournal = ({ interventions: propsInterventions, onEdit, onDelete, onCloturer, onViewDetails }) => {
  const [interventions, setInterventions] = useState(propsInterventions || []);
  const [filteredInterventions, setFilteredInterventions] = useState([]);
  const [loading, setLoading] = useState(!propsInterventions);
  const [filterDate, setFilterDate] = useState('all');
  const [filterTechnicien, setFilterTechnicien] = useState('all');

  useEffect(() => {
    // Si les interventions sont passées en props, les utiliser directement
    if (propsInterventions) {
      setInterventions(propsInterventions);
    } else {
      // Sinon les charger
      loadInterventions();
    }
